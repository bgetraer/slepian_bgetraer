...
if nargin==2 && ~all(cmn(:)==0)
  ...
  elseif iscell(c11)
    % BGETRAER: If the region is buffered it calculates the buffer area
    % ONLY!! This will return the WRONG AREA! We want to take the outline
    % of the original region out of the buffered outline!!
    
    % Specify the region by name
    defval('N',10);
    region=c11{1};
    buffer=c11{2};
    % BGETRAER find the coordinates XY for the buffered region
    eval(sprintf('XYbuff=%s(%i,%f);',region,N,buffer));
    % look for the NAN values and crop off the non-buffered outline!!!
    XY = XYbuff(1:find(isnan(XYbuff),1)-1,:);
    ...
    XY=c11;
    %Bgetraer: look for NAN values and crop off the non-buffered outline if
    %it is there!!!!
    if any(isnan(XY(:)))
        XY = XY(1:find(isnan(XY),1)-1,:);
    end
    region=XY;
    N=NaN;
  end
